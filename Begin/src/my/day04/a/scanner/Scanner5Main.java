	...
	public static void main(String[] args) {
		...
		try {
			...
			inputStr = sc.nextLine(); // "25"
				   					 // "똘똘이" (정수를 입력해야 하는데 그 외의 것을 입력했을 경우의 수)
			
			// "문자열" 을 int 타입으로 형변환 시켜주는 첫번째 방법
			int num1 = Integer.parseInt(inputStr);
			
			System.out.print(">> 두번째 정수를 입력 : ");
			inputStr = sc.nextLine(); // "2"
				   					 // "안녕" (정수를 입력해야 하는데 그 외의 것을 입력(inputStr으로)했을 경우의 수)
			
			int num2 = Integer.parseInt(inputStr);
			
		//  [참고] int 타입을 문자열로 형변환 시켜주는 두번째 방법
			String str_num1 = Integer.toString(num1); //int를 String으로 바꿔주는 것
			...
			double result = 0;			//int result = 0;에서 double result = 0; 으로 바꿔도 됨 (나머지 계산 결과할 때 소수점이 나오게 하기 위함.) // 정수는 실수로 자동 형변환된다. (int→double)
			boolean b_flag = false;
			
			switch (operator) {			//operator에는 float와 double이 들어올 수 없다.
				case "+":
					result = num1 + num2;
			...
			if(b_flag == false) { //operator가 "+" 또는 "-" 또는 "*" 또는 "/" 인 경우 
				...
				if(operator.equals("/")) { //"문자열1" 과 "문자열2" 의 값이 같은지 다른지를 비교할 때는 == 또는 !=를 쓰면 안된다.
										   //"문자열1".equals("문자열2")을 사용한다.★★★★★★★★★★★★★★★★★★★★★★★★★★★
										   //만약에 "문자열1" 과 "문자열2"의 값이 같다면 "문자열1".equals("문자열2") 은 true가 되고, 
										   //만약에 "문자열1" 과 "문자열2"의 값이 다르다면 "문자열1".equals("문자열2") 은 false가 된다.
				...
				System.out.println(">> "+operator+" 은(는) 사칙연산(+ - * /)가 아니므로 계산이 불가합니다.!! <<");
			}
