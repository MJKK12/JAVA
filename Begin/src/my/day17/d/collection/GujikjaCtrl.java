...
public class GujikjaCtrl extends Controller implements InterGujikjaCtrl { // Controller가 부모 (Controller를 상속받아옴)
// interface를 넣은 이유 :: 목차 기능 (보기 좋으라고 넣었음. 유지보수가 편하다. 바로바로 찾아갈 수 있음.)
// 이름이 다르면 헷갈리기 때문에, 부모(Controller)를 받아옴. 
// ▶ 상속을 똑같이 받아올 수 있음. 중복을 피할 수 있음. (로그인/비밀번호 부분) 구현되어진 클래스를 상속받아온다.
	...
	public void register(Scanner sc, List<Member> mbrList) {
			...
			// List는 방이 무제한이기 때문에, 방이 몇개인지 알아볼 필요 X
			...
			for(;;) {
				...
				for(int i=0; i<mbrList.size(); i++) {	
					String stored_userid = mbrList.get(i).getId(); //mbrArr[i] 에서 바꿈
	...
	public void showAllGujikja_info(List<Member> mbrList) {
		...
		for(int i=0; i<mbrList.size(); i++) {
			...
			if( mbrList.get(i) instanceof Gujikja &&	// mbrList.get(i) 에서 끄집어 내온 애가 Gujikja입니까?
