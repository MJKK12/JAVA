...
/*
 ...
 ★ 5. 일반적인 데이터 추가/삭제는 데이터 중간 중간마다 발생하므로 이러한 경우에는 LinkedList 가 ArrayList 보다 상대적으로 빠르다.    
   ...
   7. LinkedList 보다는 ★ArrayList 를 사용★하도록 하자. 
   // 어떤 제품의 물건(데이터)을 올린 다음에는 삭제하는 경우가 드물다. (계속 팔것이기 때문에!!) ▶ 중간에 끼어들 때에는 Linked가 낫지만, 그게 아니라면 ArrayList가 낫다.
   
   == LinkedList (저장소) ==   
   													▼자신의 메모리 주소와 함께 앞의 메모리 주소가 "연결"(linked) 됨▼
