...
public class ArrayList_main_1 {
	...
	public static void main(String[] args) {
	 ...
	 2. 자료구조(Collection)은 동일한 타입의 객체가 아니더라도
	    ...
	    웹에서 가장 많이 사용하는 대표적인 Collection 타입은 ★List 계열과 Map 계열★이다.

	-------------------------------------------------------------------------
	              Web에서 주로 사용하는 클래스     게임프로그래밍에서 주로 사용하는 클래스
	Interface     (Single Thread 방식-단일)        (Multi Thread 방식)
	-------------------------------------------------------------------------           
	List 계열    --      ArrayList      ,         Vector
	Map  계열    --      HashMap        ,         Hashtable 

    ArrayList 및 HashMap 은 Multi Thread를 지원안해주므로 가볍다. (메모리를 덜 차지한다.)
    Vector 및 Hashtable 이 Multi Thread를 지원해주므로 무겁다.

    ★Multi Thread를 지원해주느냐 안해주느냐만 차이★가 있을 뿐
    그 나머지 기능은 동일하다.  
	*/

	/*
	   // 원래 객체만 들어오면 다 받아주는데, 제네릭은 ArrayList에서 어떠한 타입만 들어오게 막아버리는 것!!
	   ...
	   프로그램 실행시 에러를 발생하지 않도록 사전에 방지하는 것이 좋기 때문이다.
	     ...
	     실행하기전 컴파일 단계에서 특정한 객체 타입이 아니면 에러를 발생하도록 하여 저장이 불가능하도록 만들었다.
	     즉, 특정 컬렉션(자료구조)에 저장될 객체의 타입을 제한하도록 만든 것이다.
	...
	*/
		...
		String[] nameArr = new String[10];	// 배열 크기는 10, 방번호는 0~9. (10부터는 안됨..!!)
	...
	/*	for(int i=0; i<20; i++) {
		...
		// Exception in thread "main" java.lang.ArrayIndexOutOfBoundsException: 10 이 발생함. (오류)
	*/	
		///////////////////////////////////		
	...
	//	nameList.size() => 0		//들어온 것이 하나도 없다.

		nameList.add(new String("이순신"));
		nameList.add("엄정화");
		
		nameList.add(new Double(1234.5));
		nameList.add(9876.5);	// double --> Double (double을 Double로 자동적으로 바꿈. autoBoxing)
		
		nameList.add(new Integer(98));
		nameList.add(88);		// int --> Integer (autoBoxing)
		
		nameList.add(new Member("superman","qWer1234$","슈퍼맨"));

		System.out.println("nameList.size() => " + nameList.size() );
	//	nameList.size() => 7		// 7개가 들어왔다.
	...
	//	nameList.size() => 200007
		...
		for(int i=0; i<=nameList.size(); i++) {	// 배열 크기 ==> 배열명.length가 크기 // 콜렉션 크기 ==> List.size() 
		...
		Member mbr = new Member ("hongkd","abcd1234$","홍길동");		// 모든클래스의 부모클래스는 object임. Member클래스에서 재정의함.
		...
		*/
		...
		//ArrayList는 오로지 member 객체만 들어온다!
