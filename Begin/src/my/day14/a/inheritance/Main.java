...
*/
		guArr[Gujikja.count++] = admin_gu;		// count 가 private 가 아닌데 왜 빨간줄이 뜰까? : count가 default 이기 때문이다.
		guArr[Gujikja.count++] = lss_gu;		// 동일 패키지는 import 할 필요가 없음. 구직자 13에서 가져왔기 때문이다.(다른패키지)
		guArr[Gujikja.count++] = ejh_gu;		// 다른패키지에서 복붙해올때 맨 위에 import를 지운다.
		...
		comArr[Company.count++] = com1;	// ★배열에 다시 담아주는 작업이 필요, 이 부분이 없었기 때문에, 가입된 아이디가 있다고 해놨음에도 프로그램을 돌리면 새로운 id처럼 인식돼서 회원가입이 됐음.
		comArr[Company.count++] = com2;	// com2; 만 담아놓는 작업을 해놨기 때문에 처음에 com1을 담아두지 않았을 때에는 com1이 중복확인에서 체크가 안됐음. (가입되어있지 않은 id 처럼 인식됨)
