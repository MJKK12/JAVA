...
public class Main_3 {
	...
	public static void main(String[] args) {
		...
		emp.register(new Employee("wonderman", "0070", "원더우먼", "과장"));
		...
		*/
		...
		System.out.println("\n~~~ 와일드 카드의 하한제한 하기. T 와 그 조상 클래스 들만 사용 가능한 경우 ~~~\n");
	...
	//	box.register_executive(box_plain);	//오류 발생.!!! ▶ 평사원이 executive에 들어갈 수 없음 
	/*  ★super★일때
	...
	*/
		...
		System.out.println("\n~~~ 와일드 카드의 상한제한 하기. T 와 그 자손클래스들만 사용 가능한 경우 ~~~\n");

		// 자식의 자식 
		Employee_Child_Plain_Child plain_child_emp = new Employee_Child_Plain_Child("kangkc","1237","강감찬","사원",3000,12);
	...
	//	box.register_plain(box_emp);	// 오류발생 .!!! plain_child의 부모는 들어오지 못함.!!
	/*  ★extends★ 일 때
	 	register_plain(Box_Anything<? extends Employee_Child_Plain> item) 이므로
	 	파라미터가 Employee_Child_Plain 을 포함한 자식클래스 까지만 입력이 가능한데
	 ...
	 */
		...
		for(Box_Anything<? extends Employee_Child_Plain> b : child_plain_list) {
			...
			@SuppressWarnings("unchecked")		// 경고표시 하지 마라 (Warnings를 Suppress 해라.)
			List<Employee_Child_Plain> list = (List<Employee_Child_Plain>) b.getList();	// Casting으로 양쪽을 맞춰줌.(mismatch가 나왔기 때문에 맞춰줌)
