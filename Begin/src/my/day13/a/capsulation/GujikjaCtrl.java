...
public class GujikjaCtrl {
	...
	public void showMenu(Scanner sc, Gujikja[] guArr) {
		...
		do {			
			...
			if(login_gu != null) {			// 로그인 한 경우 (null이 아님)
				...
				if("admin".equals(login_gu.getUserid())) {			// 로그인을 했는데 ,그 아이디가 admin 입니까?
					// 관리자로 로그인 한 경우
					str_menuno_4 = "\n4. 관리자전용";
				}
			}
			...
			case "4":					// 관리자전용
				if(login_gu != null && "admin".equals(login_gu.getUserid())) {	// 로그인 한 경우 - null도 아니면서 admin이랑 id가 같은지?
					// 관리자로 로그인 한 경우
		...
		} while (!("10".equals(str_menuNo) ));			// ▶ () 안이 참이면 빠져나온다.
		...
		if(Gujikja.count < guArr.length) { 						// 들어가려면 방이 있어야 함..
			...
			for(;;) {
				...
				for(int i=0; i<Gujikja.count; i++) {
					String stored_userid = guArr[i].getUserid();			// 저장된 정보들(이미 가입된)이 있는 guArr[i]
			...
			gu.setUserid(userid);				// 개발자가 의도한대로 userid가 조건대로 들어와야 하는 것.
			gu.setPasswd(passwd);
			gu.setName(name);
			gu.setJubun(jubun);
	...
	private Gujikja login(Scanner sc, Gujikja[] guArr) {
		...
		for(int i=0; i<Gujikja.count; i++) {
			String stored_userid = guArr[i].getUserid();	// 직접 못나오기 때문에 get을 가져온다
		...
		if(Gujikja.count == 1) {
			...
			do {
				...
				for(int i=0; i<Gujikja.count; i++) {
					...
					if(!"admin".equals(guArr[i].getUserid())) {		//admin이 아니라면 검사를 한다.
