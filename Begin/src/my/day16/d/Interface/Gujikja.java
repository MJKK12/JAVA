...
public class Gujikja extends Member { // 자식클래스(부모클래스인 Member에서 다 상속을 받아와야 한다.)
...
//	static int count; // Gujikja 객체(인스턴스)의 개수를 알아오려는 용도 
	...
	// == 구직자 1명의 정보를 보여주는 메소드 생성하기 == //	// Interface가 있는 것 (InterMember에서 아웃라인만 주면, 그것을 받아와서 재정의를 하는 것이다.) 
	//(팀장님이 인터페이스에서 정해놓은 메소드명을 그대로 사용해야한다!!)
	@Override
