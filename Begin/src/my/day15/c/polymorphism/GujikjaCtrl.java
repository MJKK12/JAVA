...
public class GujikjaCtrl {
	...
	private void register(Scanner sc, Member[] mbrArr) {	// 회사 아이디와 구직자 아이디가 합쳐져버림★
	...
	private Gujikja login(Scanner sc, Member[] mbrArr) {
		...
		Gujikja login_gu = null;	// 부모클래스에서는 자식클래스 받을 수 있지만, 반대는 안된다.
		...
		for(int i=0; i<Member.count; i++) {
			...
			if( stored_userid.equals(userid) && stored_passwd.equals(passwd) ) {
				login_gu = (Gujikja) mbrArr[i]; //▶ Gujikja로 강제 형변환 해준다.(Gujikja는 자식 class)
			}
	...
	// 관리자를 제외한 모든 구직자 정보를 조회해주는 메소드 생성하기 // 이 메소드는 admin으로 로그인을 해야만 볼 수 있는것임
	private void showAllGujikja_info(Member[] mbrArr) {
		
		int count_gu = 0; // 구직자의 count (갯수) 를 알고 싶다.
		
		for(int i=0; i<Member.count; i++) {	// 끄집어 내자!
		...
		if(count_gu == 0) {	// admin을 뺀 나머지가 없는 것 (구직자는 구직자인데, 그 id가 admin)
		   // mbrArr 배열에 저장되어진 객체정보 중 Gujikja 객체가 관리자1개(admin)와 Compnay 2개가 저장된 경우
		   System.out.println(">> 등록되어진 구직자가 아무도 없습니다. << \n");	
			...
			for(int i=0; i<Member.count; i++) {
				...
				if(mbrArr[i] instanceof Gujikja &&
					!"admin".equals(mbrArr[i].getId()) ) {
					mbrArr[i].viewInfo();	// 배열 내에 Company와 Gujikja이지만, 부모 클래스타입인 Member를 받아옴.
											// ▶ 이 자체를 Gujikja로 바꿔준다. why? 배열은 저장타입이 Member인데, viewInfo라는 메소드가 없음.
											// 그렇기 때문에 .찍어도 viewinfo();가 나오지 않음. 
											// 방법 두가지 ① Gujikja로 캐스팅 ② viewInfo를 추상 메소드로 만들기. (Member 부모 클래스에 추상메소드 만들어줌)
